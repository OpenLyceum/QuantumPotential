/**
 * Solver robustness: for random slider settings on every screen, with every potential and every
 * numerical method from Preferences, the bound states must be finite numbers (or absent, e.g. a
...
import QPPWPreferences from "../src/preferences/QPPWPreferencesModel.js";
...
describe("solver robustness", () => {
  afterEach(() => {
    QPPWPreferences.numericalMethodProperty.reset();
    QPPWPreferences.gridPointsProperty.reset();
  });

  it.each(MODELS)("%s gives finite bound states for random settings", (_name, create, potentials) => {
    const model = create();
    const random = makeRandom(20260922);
    const methods = Object.values(NumericalMethod);
    const failures: string[] = [];

    for (let trial = 0; trial < TRIALS; trial++) {
      model.potentialTypeProperty.value = potentials[Math.floor(random() * potentials.length)]!;
      QPPWPreferences.numericalMethodProperty.value = methods[Math.floor(random() * methods.length)]!;
      QPPWPreferences.gridPointsProperty.value = GRID_SIZES[Math.floor(random() * GRID_SIZES.length)]!;

      const setting: string[] = [model.potentialTypeProperty.value, QPPWPreferences.numericalMethodProperty.value];
